...
from MailingList.models import MailingList, Client, Message
from MailingList.serializers import MailingListSerializer, ClientSerializer, MessageSerializer
...
class MailingListViewSet(viewsets.ModelViewSet):
    ...
    def info(self, request, pk=None):
        ...
        """
        queryset_mailing = MailingList.objects.all()
        get_object_or_404(queryset_mailing, pk=pk)
        queryset = Message.objects.filter(mailing_list_id=pk).all()
    ...
    def fullinfo(self, request):
        ...
        for row in mailing:
            ...
            mail = Message.objects.filter(mailing_list_id=row['id']).all()
