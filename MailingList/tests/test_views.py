...
class TestStat(APITestCase):
    ...
    def test_mailing(self):
        mail_count = MailingList.objects.all().count()
        mail_create = {'date_start': now(), 'date_end': now(), 'time_start': now().time(),
                       'time_end': now().time(), 'message': "Simple text", "tag": "crazy",
                       'mobile_operator_code': '412'}
        response = self.client.post('http://127.0.0.1:8000/api/mailings/', mail_create)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(MailingList.objects.all().count(), mail_count + 1)
        self.assertEqual(response.data['message'], 'Simple text')
